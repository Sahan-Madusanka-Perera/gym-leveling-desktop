...
export async function addMember(formData: FormData) {
  ...
  console.log("ADD MEMBER: Starting to add member");
  ...
  try {
    ...
    const memberData = {
      ...
      // Explicitly omit member_id as it should be generated by the database
    }

    console.log("ADD MEMBER: Prepared data:", memberData);

    // Comprehensive input validation
    if (!memberData.first_name) {
      console.error("ADD MEMBER: Missing first name");
      throw new Error("First name is required")
    }
    if (!memberData.last_name) {
      console.error("ADD MEMBER: Missing last name");
      throw new Error("Last name is required")
    }
    if (!memberData.gender) {
      console.error("ADD MEMBER: Missing gender");
      throw new Error("Gender is required")
    }
    if (!memberData.phone_number) {
      console.error("ADD MEMBER: Missing phone number");
      throw new Error("Phone number is required")
    }
    if (!memberData.birth_date) {
      console.error("ADD MEMBER: Missing birth date");
      throw new Error("Birth date is required")
    }

    console.log("ADD MEMBER: All validation passed, inserting into database");
    ...
    if (error) {
      console.error("ADD MEMBER: Supabase Insertion Error:", {
      ...
      if (error.code === 'PGRST100') {
        // Try to setup RLS policies automatically
        try {
          console.log("ADD MEMBER: Attempting to setup RLS policies");
          await fetch('/api/setup-rls-policy', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          });
          
          // Retry the insert after setting up policies
          console.log("ADD MEMBER: Retrying insert after setting up RLS policies");
          ...
          if (retryError) {
            console.error("ADD MEMBER: Retry failed after RLS setup:", retryError);
            throw new Error(`Failed to add member after RLS setup: ${retryError.message}`);
          }
          
          return retryData;
        } catch (rlsError) {
          console.error("ADD MEMBER: RLS setup and retry failed:", rlsError);
          throw new Error("Permission denied: Please use the 'Setup RLS Policy' button and try again.");
        }
      }
    ...
    console.log("ADD MEMBER: Member added successfully:", data);
    return data
  } catch (error) {
    console.error("ADD MEMBER: Comprehensive error:", error instanceof Error ? error.message : "Unknown error");
    throw error
  }
}
